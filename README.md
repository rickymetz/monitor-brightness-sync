...
- **Gamma safety.** Dimming is clamped to a small visible floor by default (the "Allow dimming all the way to black" option removes the clamp for true blackout); gamma is restored on quit, and `CGDisplayRestoreColorSyncSettings()` runs on launch to self-heal a force-killed run.
