...
- **Software dimming can't brighten.** With no DDC channel there's no backlight to command — the gamma table only scales luminance downward, so set the monitor's own buttons to maximum and the app's 100% becomes that.
- **Going all the way to black on a software-dimmed display.** By default these displays stop at a visible floor, because the screen you'd need in order to undo the dimming is the one going dark. Turning on "Allow dimming all the way to black" lifts that floor and lets them reach true black; the brightness keys still bring them back. Note where black lands: it's the point your calibration curve maps to 0, which with the default curve is the built-in at 15% and below. If you want black only at the very bottom of the range, recalibrate that monitor's curve.
